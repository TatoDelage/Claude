...
export default function LyricsLabPage() {
  ...
  const lyricsReady = Boolean(health?.lyricsProviderConfigured);
  const providerLabel = health?.lyricsProvider === "lrclib" ? "LRCLIB (desarrollo)" : health?.lyricsProvider ?? "Letras";
    ...
    <main className="game-stage min-h-screen px-4 py-10">
      <div className="max-w-3xl mx-auto space-y-6">
        <header>
          ...
          <p className="game-muted mt-3">Comprueba una palabra o frase contra LRCLIB. La letra se procesa solo en memoria y nunca se guarda en el catálogo.</p>
        ...
        <section className="game-panel rounded-[2rem] p-5">
          ...
          <div className="grid sm:grid-cols-2 gap-2 text-sm">
            ...
            <Status label={providerLabel} ok={lyricsReady} />
          ...
          <p className="mt-3 text-sm game-muted">LRCLIB es gratuito y no necesita API key. Lo usamos para validar arquitectura y cobertura; el proveedor comercial de producción se decidirá más adelante.</p>
        </section>
        ...
        <form onSubmit={search} className="game-panel rounded-[2rem] p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <label className="block">
              ...
              <input value={title} onChange={(event) => setTitle(event.target.value)} className="mt-2 w-full rounded-xl bg-black/20 border border-canvas-600 px-4 py-3" placeholder="Ej. Soldadito marinero" />
            </label>
            <label className="block">
              <span className="text-xs font-black uppercase tracking-wider game-muted">Artista</span>
              <input value={artist} onChange={(event) => setArtist(event.target.value)} className="mt-2 w-full rounded-xl bg-black/20 border border-canvas-600 px-4 py-3" placeholder="Ej. Fito y Fitipaldis" />
            </label>
          </div>
        ...
        {selected && (
          <section className="game-panel rounded-[2rem] p-5 space-y-4">
            <div>
              ...
              <p className="game-muted text-sm mt-1">Spotify aporta título, artista, álbum y duración para buscar una coincidencia precisa en LRCLIB.</p>
            </div>
            <label className="block">
              <span className="text-xs font-black uppercase tracking-wider game-muted">Palabra o frase exacta</span>
              <input value={query} onChange={(event) => setQuery(event.target.value)} className="mt-2 w-full rounded-xl bg-black/20 border border-canvas-600 px-4 py-3" placeholder="Ej. corazón" />
            </label>
            <button type="button" onClick={verify} disabled={verifying || !lyricsReady || query.trim().length < 1} className="game-primary w-full rounded-xl py-3 font-black disabled:opacity-40">
        ...
        {lyricsResult && (
          <section className={`rounded-[2rem] border p-5 ${lyricsResult.status === "verified" ? "border-emerald-700/40 bg-emerald-950/20" : lyricsResult.status === "rejected" ? "border-rose-700/40 bg-rose-950/20" : "border-amber-700/40 bg-amber-950/20"}`}>
            <p className="game-kicker">Resultado · {lyricsResult.provider.toUpperCase()}</p>
