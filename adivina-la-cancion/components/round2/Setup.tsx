...
export default function Setup({
  ...
  teams: Team[];
  onStart: (setup: Round2Setup) => void;
}) {
      ...
      <div className="flex-1 overflow-auto px-4 py-5 space-y-6 max-w-2xl mx-auto w-full">
        ...
        <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
          ...
          <div className="flex gap-3 text-sm">
            ...
            <div className="flex-1 text-center bg-zinc-800 border border-zinc-700 rounded-xl py-2.5">
        ...
        <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 space-y-4">
          ...
          <DurationControl
            label="Respuesta"
            hint="Tiempo para responder (y para rebotes)"
          ...
          {teams.map((team) => (
            <div
              ...
              className={`flex-1 h-1.5 rounded-full ${
                teamComplete(team.id) ? TEAM_ACCENT[team.color] : "bg-zinc-800"
              }`}
            />
          ))}
        </div>
        ...
        >
          {allComplete ? "¡Empezar Ronda 2! 🎵" : "Rellena todas las canciones"}
        </button>
