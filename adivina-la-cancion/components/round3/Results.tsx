...
import { TurnResult, scoresByTeam, POINTS_CORRECT, POINTS_FAIL } from "@/lib/round3";
    ...
    <div className="min-h-screen bg-canvas-950 text-white flex flex-col">
      <header className="px-4 pt-6 pb-4 border-b border-canvas-700/60 text-center">
        ...
        <p className="text-zinc-400 text-sm mt-1">
          {totalCorrect} de {results.length} canciones acertadas a la primera
        </p>
      </header>
      ...
      <div className="flex-1 overflow-auto px-4 py-6 space-y-5 max-w-2xl mx-auto w-full">
        ...
        <section className="bg-canvas-900 border border-canvas-700 rounded-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-canvas-700">
            <p className="text-sm font-bold text-zinc-300">Canción a canción</p>
          </div>
          <div className="divide-y divide-canvas-700/60">
            {results.map((r, i) => {
              const buzzer = teams.find((t) => t.id === r.buzzerTeamId)!;
              const label = [r.song.title, r.song.artist].filter(Boolean).join(" — ") || "Sin info";
              return (
                <div key={i} className="px-4 py-3 space-y-1.5">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className={`text-xs ${TEAM_TEXT[buzzer.color]}`}>🔔 {buzzer.name}</p>
                    </div>
                  ...
                  {r.rebounds.filter((rb) => rb.choice !== "pass").map((rb) => {
                    ...
                    return (
                      <div key={rb.teamId} className="flex items-center justify-between gap-3 pl-3 border-l-2 border-canvas-700">
                        <p className={`text-xs ${TEAM_TEXT[rbt.color]}`}>↩ {rbt.name}</p>
